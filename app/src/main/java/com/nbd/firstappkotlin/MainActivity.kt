package com.nbd.firstappkotlin

import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import android.util.Log
import javax.security.auth.login.LoginException
import kotlin.math.log

class MainActivity : AppCompatActivity() {
    val constante: String= "una constante"
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_main)

        val a: Int=1 //constante a

        var b:Int =3 //variable
        b=5
        Log.i("Basics", "el valor de b es: $b")
        Log.i("Basics", constante)
        imprimir()
        unSwitch()
        unIf(1)
        unFor()
        unosWhile()
    }
    fun imprimir(){
        Log.i("Basics", constante)

    }

    fun unSwitch(){
        val x=1

        when(x){
            1-> Log.i("Basics", "Es uno ")
            2->{
                var algo="lorem"
                var otro="ipsum"
                Log.i("Baiscs", algo+otro )
            }
            else -> Log.i("Basics", "no coincidio")
        }
    }

    fun unIf (a:Int):Boolean{
        var array=arrayOf(1,2,3,4,5)
        if(a in array)
            return true
        else
            return false
    }
    fun unFor(){
        var array=arrayOf(1,2,3,4,5)
        for (numero in array){ //array.indices es para el lugar del numero en arreglo
            Log.i("Basics",numero.toString())
        }
        Log.i("Basics", "1..5")
        for (i in 1..5){
            Log.i("Basics", i.toString())
        }

        Log.i("Basics", "6 dowwnTo 0 step 2")
        for (i in 6 downTo 0 step 2) {
            Log.i("Basics", i.toString())
        }

    }

    fun unosWhile(){
        var x=0
        while(x<5){
            Log.i("Basics",x.toString())
            x++
        }

        do{
            Log.i("Basics",x.toString())
            x++
        }while (x<5)
    }
}
